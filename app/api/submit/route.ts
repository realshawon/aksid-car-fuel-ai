...
export async function POST(request: NextRequest) {
  try {
    ...
    if (payload.fuelBillBase64 && isImage(payload.fuelBillType ?? '', payload.fuelBillFileName ?? '')) {
      ...
      if (!fuelAnalysis) {
        undetectedFields.push('Fuel Bill — could not analyze image (AI unavailable or read error)');
        ...
        (fuelAnalysis.cannot_read as string[]).forEach((f: string) =>
          undetectedFields.push(`Fuel Bill — ${f}`)
        );
      }
    } else if (payload.fuelBillBase64) {
      // PDF — AI vision doesn't support PDF, note it
      undetectedFields.push('Fuel Bill is a PDF — auto-analysis not available, please review manually');
    }
    ...
    if (token && payload.fuelBillBase64) {
      try {
        ...
        fuelBillUrl = await uploadToGitHub(token, name, payload.fuelBillBase64,
          `${payload.driver} – Fuel Bill – ${formatDate(dateStr)}`);
      } catch (e) {
        console.error('[submit] fuelBill upload failed:', e);
        undetectedFields.push('Fuel Bill attachment — upload to GitHub failed');
      }
    }
    ...
    const emailHtml = buildEmailHtml(row, fuelBillUrl, logSheetUrl, fuelAnalysis, logAnalysis, undetectedFields);
