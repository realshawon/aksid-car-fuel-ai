...
export async function POST(request: NextRequest) {
  try {
    ...
    if (payload.fuelBillBase64 && isImage(payload.fuelBillType ?? '', payload.fuelBillFileName ?? '')) {
      ...
      if (!fuelAnalysis) {
        undetectedFields.push('Fuel Bill â could not analyze image (AI unavailable or read error)');
        ...
        (fuelAnalysis.cannot_read as string[]).forEach((f: string) =>
          undetectedFields.push(`Fuel Bill â ${f}`)
        );
      }
    } else if (payload.fuelBillBase64) {
      // PDF â AI vision doesn't support PDF, note it
      undetectedFields.push('Fuel Bill is a PDF â auto-analysis not available, please review manually');
    }
    ...
    if (token && payload.fuelBillBase64) {
      try {
        ...
        fuelBillUrl = await uploadToGitHub(token, name, payload.fuelBillBase64,
          `${payload.driver} â Fuel Bill â ${formatDate(dateStr)}`);
      } catch (e) {
        console.error('[submit] fuelBill upload failed:', e);
        undetectedFields.push('Fuel Bill attachment â upload to GitHub failed');
      }
    }
