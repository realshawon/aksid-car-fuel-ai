...
// Give the function enough time to upload photos + run AI on mobile-sized images.
// Without this, Vercel kills the request at ~10s and the app hangs on "AI Processing".
  ...
  try {
    ...
    for (let i = 0; i < fuelBillFiles.length; i++) {
      ...
      if (isImage(fb.type, fb.name)) {
        ...
        undetectedFields.push(`${label} is a document file (PDF/DOC) — auto-analysis not available, please review manually`);
    ...
    if (payload.logSheetBase64 && isImage(payload.logSheetType ?? '', payload.logSheetFileName ?? '')) {
      ...
      undetectedFields.push('Driver Log Sheet is a document file (PDF/DOC) — auto-analysis not available, please review manually');
    }
    ...
    const row: Record<string, any> = {
      ...
      fuelBillTotalKB:    fuelBillFiles.reduce((s, f) => s + (+((f as unknown as Record<string,number>).compKB) || 0), 0) || null,
